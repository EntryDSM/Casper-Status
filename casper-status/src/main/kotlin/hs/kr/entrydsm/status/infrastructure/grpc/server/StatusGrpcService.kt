...
class StatusGrpcService(
    ...
    override suspend fun getStatusList(request: Empty): StatusServiceProto.GetStatusListResponse {
        ...
        val internalStatusResponseList =
            statusList.map {
     ...
     */
    override suspend fun getStatusByReceiptCode(
        request: StatusServiceProto.GetStatusByReceiptCodeRequest,
    ): StatusServiceProto.GetStatusByReceiptCodeResponse {
        val status = getStatusByReceiptCodeUseCase.execute(request.receiptCode)

        val internalStatusResponse =
            InternalStatusResponse(
                id = status.id,
